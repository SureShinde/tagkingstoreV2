...
class Unirgy_RapidFlowPro_Model_Mysql4_ProductExtra extends Unirgy_RapidFlow_Model_Mysql4_Catalog_Fixed
{
    ...
    protected function _importRowCPI($row)
    {
        ...
        $imgId = $this->_write->fetchOne("select value_id from " . $t . " where attribute_id=" . $galleryAttrId . " and `entity_id`=" . $pId . " and binary `value`=" . $this->_write->quote($imgFilename));
        if( !$imgId && $imgFileChanged ) 
        {
            $img = array( "attribute_id" => $galleryAttrId, "entity_id" => $pId, "value" => $imgFilename );
    ...
    protected function _fetchCategoryRow($urlPath)
    {
        ...
        if( !$this->_urlPaths ) 
        {
            ...
            foreach( $rows as $r ) 
            {
                if( $noUrlPath ) 
                {
                    $r["url_path"] = $this->catBuildPath($r);
                }
