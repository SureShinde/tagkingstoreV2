...
class Unirgy_RapidFlowPro_Model_Mysql4_Category_Abstract extends Unirgy_RapidFlow_Model_Mysql4_Catalog_Abstract
{
    ...
    protected function _importFetchOldData()
    {
        ...
        {
            $select->joinLeft(array( "up" => $table . "_varchar" ), "up.entity_id=e.entity_id and up.attribute_id=" . $upAttrId, array( "value_id", "url_path" => "value", "store_id" ))->where("e.entity_id in (?)", $entityIds)->where("up.store_id in (0, ?)", $storeId);
        }

        $this->_attrJoined = array( $upAttrId );
        $categoryRows = $this->_write->fetchAll($select);
        Mage::log((string) $select, NULL, "rf.log", true);
        Mage::log($categoryRows, NULL, "rf.log", true);
        unset($select);
        ...
        foreach( $categoryRows as $r ) 
        {
            ...
            if( $noUrlPath ) 
            {
                $r["url_path"] = isset($this->_categories[$r["entity_id"]]["url_path"]) ? $this->_categories[$r["entity_id"]]["url_path"] : $this->catBuildPath($r);
            }
            ...
            $this->_attrValueIds[$r["entity_id"]][$sId]["url_path"] = $r["value_id"];
            $this->_entities[$r["entity_id"]][$sId] = $r1;
        }
    ...
    protected function _fetchAttributeValues($storeId, $defaults = false, $entityIds = NULL, $limitAttrIds = NULL)
    {
        ...
        foreach( $this->_attributesByType as $type => $attrs ) 
        {
            ...
            $select = $this->_read->select()->from($table . "_" . $type)->where("entity_id in (?)", $entityIds)->where("attribute_id in (?)", $attrIds)->where("store_id in (0, ?)", $storeId);
            $rows = $this->_read->fetchAll($select);
            if( empty($rows) ) 
            {
                Mage::log("rows empty: " . 624, NULL, "rf.log", true);
                continue;
            }
        ...
        foreach( $this->_newData as $urlPath => $p ) 
        {
            ...
            foreach( $p as $k => $newValue ) 
            {
                ...
                if( $isNew && !$empty || $isValueChanged ) 
                {
                    ...
                    if( !$isNew ) 
                    {
                        $logger->setColumn(isset($this->_fieldsCodes[$k]) ? $this->_fieldsCodes[$k] + 1 : -1)->success(NULL, NULL, $newValue, $oldValue);
                    }
    ...
    protected function _importSaveAttributeValues()
    {
        ...
        foreach( $this->_insertAttr as $type => $attrs ) 
        {
            ...
            foreach( $chunks as $chunk ) 
            {
                ...
                {
                    $this->_profile->getLogger()->error("Unirgy_RapidFlowPro_Model_Mysql4_Category_Abstract::_importSaveAttributeValues" . ":" . 1249 . ":" . $e->getMessage());
                    Mage::log($sqlPrefix . join(",", $chunk), NULL, "rf.log", true);
                    Mage::log($attrs, NULL, "rf.log", true);
                }
            }
        }
        foreach( $this->_updateAttr as $type => $attrs ) 
        {
            ...
            foreach( $attrs as $k => $v ) 
            {
                ...
                {
                    $this->_profile->getLogger()->error("Unirgy_RapidFlowPro_Model_Mysql4_Category_Abstract::_importSaveAttributeValues" . ":" . 1265 . ":" . $e->getMessage());
                }
            }
        }
        foreach( $this->_deleteAttr as $type => $vIds ) 
        {
            ...
            {
                $this->_profile->getLogger()->error("Unirgy_RapidFlowPro_Model_Mysql4_Category_Abstract::_importSaveAttributeValues" . ":" . 1276 . ":" . $e->getMessage());
            }
        }
    }
    ...
    protected function getUrlPaths($storeId, $table)
    {
        ...
        if( $noUrlPath ) 
        {
            foreach( $rows as &$r ) 
            {
                $urlPath = $this->catBuildPath($r, "url_key", "url_key");
